...
1. Install [NodeJS 24](https://nodejs.org/en/) (see `.nvmrc`)
2. Enable [pnpm](https://pnpm.io/installation), e.g. via `corepack enable pnpm`
...
6. Install dependencies with `pnpm install` in the `desktop` directory
7. Run `pnpm tauri dev` in the `desktop` directory
...
Make sure all of your dependencies are installed and up to date by running `pnpm install`
...
To spin up the application in development mode, run `pnpm tauri dev`. It will
...
Enable debug logging to stdout during development with `DEBUG=true pnpm tauri dev`.
...
`pnpm tauri build`. You can find the packaged version of the application in the
`src-tauri/target/release/{PLATFORM}` folder.

## Check Type Errors

Run `pnpm types:check` to check for errors
...
You can upgrade the version manually or run `pnpm version ...`

## Build desktop app

If your development environment is setup successfully and you're able to run
`pnpm desktop:dev` without problems, you also should be able to build the app
